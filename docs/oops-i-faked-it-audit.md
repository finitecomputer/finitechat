...
| `containers/agent/echo_agent.py` | Container wiring and adapter importability | Hermes behavior or product chat quality | Echo container smoke test |
...
The repo-local command for that path is:
...
- Echo media tests keep their names/docs honest or are replaced by real gateway
  tests.
- A simulator proof captures a back-and-forth app conversation with real Hermes,
  including media, through `scripts/hermes-real-gateway-demo.sh` or its
  successor.
