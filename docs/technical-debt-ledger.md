...
| Chat transcript UI ahead of product projection commands | The iOS transcript renders projected reactions, read receipts, reply ids, media references, and delivery states, but Rust does not yet expose app actions for sending reactions/read receipts, paged transcript loading, or outbound media/blob uploads from iOS | UI affordances can drift into dead controls or local-only behavior if Swift starts inventing state outside the room log | The first Pika-style transcript slice is render-only for unsupported surfaces: `ChatTranscriptView` is collection-backed, uses stable row ids, and only displays fields projected by Rust | Rust owns paged transcript APIs, reaction/read-receipt commands, and attachment upload/send actions; Swift controls become thin calls into those actions and the debt row is removed |
